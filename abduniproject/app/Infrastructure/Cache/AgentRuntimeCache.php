...
final class AgentRuntimeCache {
 ...
 public static function resolve(int $agentId,string $appId): array {
  ...
  try{ Cache::tags(['ai_runtime'])->put($k,$val,self::TTL);}catch(\Throwable){}
 ...
 public static function invalidate(int $agentId,string $appId): void { Cache::forget(self::key($agentId,$appId)); try{Cache::tags(['ai_runtime'])->flush();}catch(\Throwable){} }
}

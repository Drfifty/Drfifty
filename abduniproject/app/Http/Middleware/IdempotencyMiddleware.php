...
final class IdempotencyMiddleware {
 ...
 public function handle(Request $request, Closure $next): Response {
  ...
  // FIX-360-16: app-scoped idempotency — X-App-Id partitioned
  $appId=$request->attributes->get('app_id') ?? $request->header('X-App-Id') ?? 'AU BUSINESS';
  if(!in_array($appId,['AU BUSINESS','AU MED','AU DEALS','AU SERV','AU INVEST'],true)) $appId='AU BUSINESS';
  // atomic replay check — SELECT ... FOR UPDATE style via row lock inside check
  $existing=DB::table('idempotency_keys')->where('idempotency_key',$key)->where('user_id',$userId)->where('endpoint',$endpoint)->where('app_id',$appId)->where('expires_at','>',now())->first();
  if($existing){
  ...
  try{
   if($response->getStatusCode() < 400){
    ...
    // FIX-360-16: app-scoped unique uk_idem_user_endpoint_app
    DB::table('idempotency_keys')->updateOrInsert(
     ['idempotency_key'=>$key,'user_id'=>$userId,'endpoint'=>$endpoint,'app_id'=>$appId],
     ['request_hash'=>$hash,'response_status'=>$response->getStatusCode(),'response_body'=>$bodyOut,'expires_at'=>now()->addHours(24),'created_at'=>now()]
    );
   }
  }catch(\Throwable){}

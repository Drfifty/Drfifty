...
- **AU BUSINESS Hub Anchor (AUDIT FIX):** `AU BUSINESS (ab_)` is Master Core B2B — owns `Domain/Wallet|Escrow|Calibrator` + `Services/Rules|Security|Agents|Calibrator` shared kernels; 4 spokes `AU MED/DEALS/SERV/INVEST` are Domain Bounded Contexts under AU BUSINESS vault — `CheckModuleStatus` + `HasAppIdScope` enforce tenancy.

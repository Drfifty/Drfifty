...
CREATE TABLE `service_providers` (
  ...
  CONSTRAINT `chk_provider_rating` CHECK (`rating`>=0 AND `rating`<=5), CONSTRAINT `fk_provider_user` FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON DELETE SET NULL
...
CREATE TABLE `service_tickets` (
  ...
  CONSTRAINT `chk_ticket_module` CHECK (`module_id` BETWEEN 1 AND 9), CONSTRAINT `fk_ticket_requester` FOREIGN KEY (`requester_id`) REFERENCES `users`(`id`) ON DELETE CASCADE, CONSTRAINT `fk_ticket_provider` FOREIGN KEY (`provider_id`) REFERENCES `service_providers`(`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
